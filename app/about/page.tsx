...
const description = 'DSH-Eval 是面向 DeepSeek Harness（DSH）Agent 与插件的通用测评平台。了解从任务、执行到证据与维度评分的评测流程、当前支持范围和新框架进展。';
...
const steps = [
  ...
  ['组织证据', '汇集运行轨迹、环境变化与交付物，记录来源及采集缺口。'],
  ['逐标签评分', '结合标签标准、题目参考与同一份证据评分，保留理由、引用与异常。'],
  ['阅读报告', '结合有效分数、无法判断和评分错误的数量，以及证据与运行状态理解结果。'],
];
      ...
      <main id="main-content" className="content-page reading-page product-intro-page">
        ...
        <div className="product-intro-body">
          ...
          <section className="product-intro-section" id="method-design" aria-labelledby="design-title">
            <header className="reading-section-heading">
              <h2 className="section-title" id="design-title" data-site-title="section">从任务到有依据的评分</h2>
              <p>新框架围绕以下五个环节组织评测；现有公开报告继续以各自的方法与评分规则为准。</p>
            </header>
