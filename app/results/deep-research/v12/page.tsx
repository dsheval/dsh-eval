...
export const metadata: Metadata = {
  title: `深度研究评测 · ${researchPublicationLabel} · DSHEval`,
  description: '7 个 DSH 研究插件与原生基线的小样本评测：短事实、长报告与派生诊断，逐题比较完成状态、增量和资源用量。',
  alternates: { canonical: researchUrl },
  openGraph: { title: '研究插件，能交出完整报告吗？· DSHEval', url: researchUrl, type: 'article', images: [] },
  twitter: { card: 'summary', title: `深度研究评测 · ${researchPublicationLabel} · DSHEval`, images: [] },
};
...
export default function ResearchResultsPage() {
  return <>
    ...
    <main id="main-content" className="content-page report-page benchmark-report research-report">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Dataset', name: `DSHEval 深度研究评测 · ${researchPublicationLabel}`, version: 'V12', description: 'C0 与七个插件的 40 条脱敏记录，包含派生诊断；本轮复用部分已验证的历史记录。', url: `https://dsheval.ai${researchUrl}`, datePublished: researchDate, creator: { '@type': 'Organization', name: 'DSHEval' }, distribution: { '@type': 'DataDownload', encodingFormat: 'application/json', contentUrl: `https://dsheval.ai${researchDownloadUrl}/results.json` } }) }} />
      <InnerPageHero eyebrow={`DEEP RESEARCH EVALUATION · ${researchDate}`} title={<><span className="title-phrase">研究插件，</span><wbr /><span className="title-phrase">能交出完整报告吗？</span></>} description="7 个插件与 DSH 原生基线，完成同一组短事实和长报告任务。4 个独立题面、1 个派生诊断，共 40 条公开记录。" breadcrumbs={[{ label: 'DSHEval', href: '/dsheval/' }, { label: '评测结果', href: '/dsheval/results' }, { label: '深度研究' }]} />
