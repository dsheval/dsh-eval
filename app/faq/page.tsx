...
const questions = [
  {
    ...
    answer: 'DSH-Eval 是面向 DeepSeek Harness（DSH）Agent 与插件的通用测评平台。产品围绕能力描述匹配测试集、任务执行、逐题证据、统一标签评分与原生基线比较设计；具体可用能力以交付物和验收记录为准。',
  },
  {
  ...
  },
  {
    ...
    answer: '等级表示证据和验证进展，不表示能力高低：从已收录、可运行、已完成测试、已提交自测，到关键结果已在独立环境复测。L1/L2/L3 则表示判定所需的证据层级，两者不能混用。',
  },
  {
