...
export default function ResearchMethodologyPage() {
  return (
    <>
      ...
      <main id="main-content" className="content-page memory-protocol-page research-protocol-page">
        <InnerPageHero
        ...
        <section className="memory-protocol-section" aria-labelledby="comparison-title">
          <header className="memory-protocol-heading">
            ...
            <h2 className="section-title" id="comparison-title">与原生能力对照</h2>
            <p>同一轮使用相同任务和运行条件，观察安装插件后带来的变化。</p>
          </header>
          <div>
            <div className="memory-protocol-tracks">
              <article className="memory-protocol-track">
                <header><span>BASELINE</span><h3>DSH 原生能力</h3><p>不安装额外研究插件，作为比较基线，不参与插件排名。</p></header>
              </article>
              <article className="memory-protocol-track memory-protocol-track-guided">
                <header><span>PLUGIN</span><h3>研究插件</h3><p>每次只安装一个待测插件。缺少必需凭证等准入问题单独记录，不记质量零分。</p></header>
              </article>
            </div>
          </div>
        </section>
        ...
        <section className="memory-protocol-section" aria-labelledby="procedure-title">
          <header className="memory-protocol-heading">
            ...
            <h2 className="section-title" id="procedure-title">怎样执行</h2>
            <p>本轮有两道短事实题、两道长报告题。另有一项诊断复用健康主题报告，不另算独立题目。</p>
          </header>
          <ol className="memory-protocol-timeline" role="list">
            {methodSteps.map(([number, title, copy]) => (
              <li key={number}>
                ...
                <div className="memory-protocol-step"><h3>{title}</h3><p>{copy}</p></div>
              </li>
            ))}
          </ol>
        </section>
        ...
        <section className="memory-protocol-section" aria-labelledby="scoring-title">
          <header className="memory-protocol-heading">
            ...
            <h2 className="section-title" id="scoring-title">怎样判分</h2>
            <p>检索答案与报告质量分别统计，不合成为一个加权总分。</p>
          </header>
          <div>
            <div className="memory-protocol-pass-rule"><h3>短事实：答案是否正确</h3><p>使用私有标准答案进行精确或可接受答案匹配，并检查是否实际发生多步检索或抓取。</p></div>
            <div className="memory-protocol-pass-rule"><h3>长报告：交付与证据是否充分</h3><p>先检查必需交付物、链接可达性和禁止项，再由固定模型、固定提示词的评分器评价事实、完整性、引用忠实度和风险。评分器不获知插件身份。</p><p>关键交付物齐全、事实和引用达到门槛且无严重编造，才算通过；可用但仍有缺项或证据不足，记为部分完成。链接能打开，不代表它支持报告中的结论。</p></div>
            <div className="memory-protocol-pass-rule"><h3>异常单独记录</h3><p>系统故障、超出能力范围和评分器故障分别标记。评分器无法完成时，保留确定性检查，暂不进入其质量排名，不记质量零分。</p></div>
          </div>
        </section>
        ...
        <section className="memory-protocol-section" aria-labelledby="evidence-title">
          <header className="memory-protocol-heading">
            ...
            <h2 className="section-title" id="evidence-title">公开材料与边界</h2>
            <p>这是固定小样本的插件比较，不代表所有研究任务，也不是上游基准的完整官方榜单。</p>
          </header>
          <div>
            <div className="memory-protocol-pass-rule"><p>公开数据保留状态、数值、资源用量和来源标记，可复算排名。私有题面、标准答案、报告正文和完整日志未公开，不能仅凭脱敏数据重新核验答案、引用或完整运行过程。</p><p>本轮重新运行多跳检索题，其余复用上一批已验证的记录；逐条标明来源。长文质量采用本地模型评分，不标作上游官方分数。</p></div>
