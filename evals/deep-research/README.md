...
V12 的 C0 + 7 个插件、40 条记录已通过白名单脱敏导出到 [`results/v12/`](./results/v12/README.md)，同时提供真实运行指标 JSON、榜单 JSON、榜单 HTML 和过程监控 HTML。这里的“公开运行数据”是移除私有文本、完整 URL、日志及本机路径后的数值/布尔字段，不包含原始回答或会话 trace。完整字段边界、文件树及复核命令见该目录 README。
